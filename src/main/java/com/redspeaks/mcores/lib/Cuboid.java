...
import com.redspeaks.mcores.McOres;
...
public class Cuboid {
    ...
    public void scatterBlocks(List<Composition> compositionList) {
        Bukkit.getScheduler().runTaskAsynchronously(McOres.getInstance(), () -> {
            Bukkit.getScheduler().scheduleSyncDelayedTask(McOres.getInstance(), () -> {
                for(int x = this.xMin; x <= this.xMax; ++x) {
                    for(int y = this.yMin; y <= this.yMax; ++y) {
                        for(int z = this.zMin; z <= this.zMax; ++z) {
                            final Block b = this.world.getBlockAt(x, y, z);
                            Composition comp = choose(compositionList);
                            b.setType(comp.getType());
                            comp.use();
                            if(comp.getCount() == 0) {
    ...
    public Composition choose(List<Composition> compositionList) {
        if(compositionList.size() > 1) {
            return compositionList.get(random.nextInt(compositionList.size()));
        }
        if(compositionList.isEmpty()) {
            return new Composition(Material.STONE, 1);
        }
